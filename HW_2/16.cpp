    ...
    for(int j = 0; j < n; j++){
        if(!status[0][j]){
            ...
            if(k == 5) {
                ...
                else if((jump[2][j-dist2] == 2)& dp[0][j] < d3){
                    dp[0][j] = d3;
                }
                if(dp[0][j] == dp[0][j-1]) jump[0][j] = 2;
            }
        ...
        if(!status[2][j]){
            ...
            if(k == 5) {
                ...
                else if((jump[0][j-dist2] == 2)& dp[2][j] < d1){
                    dp[2][j] = d1;
                }
                if(dp[2][j] == dp[2][j-1]) jump[2][j] = 2;
            }
            else dp[2][j] = max( dp[2][j], d1 );
        }
    }
    cout << max({dp[0][n-1],dp[2][n-1],dp[2][n-1]}) << "\n";
    // for(int i = 0; i < 3; i++){
    //     for(int j = 0; j < n; j++){
    //         cerr << dp[i][j] << " ";
    //     }
